...
import DetailChargesInfrastructureModal from "./DetailChargesInfrastructureModal";
import { adminHeaders } from "./adminHeaders";

// Postes pour lesquels une vision micro existe réellement.
const POSTES_AVEC_DETAILS: Record<
  string,
  "agents" | "superviseurs" | "apporteurs" | "infrastructure"
> = {
  salaires_agents: "agents",
  primes_performance: "agents",
  pool_superviseurs: "superviseurs",
  commissions_apporteurs: "apporteurs",
  charges_infrastructure: "infrastructure",
};
...
export default function BudgetDecaissementPanel() {
  ...
  const [detailModal, setDetailModal] = useState<
    "agents" | "superviseurs" | "apporteurs" | "infrastructure" | null
  >(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {detailModal === "infrastructure" && (
        <DetailChargesInfrastructureModal onClose={() => setDetailModal(null)} />
      )}
    </div>
  );
}
...
function PosteRow({
  ...
  onPayer: () => void;
  onSaved: () => void;
  onOpenDetails: "agents" | "superviseurs" | "apporteurs" | "infrastructure" | null;
  setDetailModal: (m: "agents" | "superviseurs" | "apporteurs" | "infrastructure" | null) => void;
}) {
