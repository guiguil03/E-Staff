...
export class ClasseVirtuelleService {
  ...
  private computeRoomStatus(
    ...
    return {
      ...
      withinJoinWindow,
      configured,
      roomUrl: withinJoinWindow && dailyRoomName ? dailyRoomUrl : null,
    };
  }

  async getSeanceRoom(groupeCle: string, numero: number): Promise<RoomStatus> {
    const seance = await this.findSeanceOrThrow(groupeCle, numero);
